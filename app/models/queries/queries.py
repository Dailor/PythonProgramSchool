...
def tasks_count_of_pupil_for_course(*, pupil_id,
    ...
    query = session.query(Group.id, Course.id, Group.name, Course.name, func.count(distinct(Task.id)),
                          literal(solution_status)).select_from(Group)
    query = query.join(Group.courses)
    query = query.join(Lesson, Lesson.course_id == Course.id)
    query = query.join(Task, Task.lesson_id == Lesson.id)
    query = query.outerjoin(Solution,
    ...
    query = query.group_by(Group.id, Course.id)
    query = query.order_by(Group.id, Course.id)
...
def count_tasks_solved_for_lessons_by_pupil(*, pupil_id, group_id):
    ...
    query = query.join(Solution, and_(Solution.pupil_id == pupil_id, Solution.group_id == group_id,
...
def count_tasks_solved_for_lessons_by_pupils_in_group(*, group_id):
    session = db_session.create_session()
    query = session.query(Lesson.id, Task.id, func.count(distinct(Solution.pupil_id))).select_from(Group)
    query = query.join(Group.lessons)
    query = query.join(Lesson.tasks)
    query = query.join(Solution, and_(Solution.group_id == group_id, Solution.task_id == Task.id,
                                      Solution.review_status.is_(TaskCheckStatus.ACCESS)))
